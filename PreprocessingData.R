...
data <- read.csv("RawTweets.csv")
## view(data)

data.text <- iconv(data$full_text, to = "utf-8")
...
rlog::log_info("Cleaning location data.")

data.location <- iconv(data$location, to = "utf-8")
location_corpus <- Corpus(VectorSource(data.location))

location_corpus <- tm_map(location_corpus, tolower)
location_corpus <- tm_map(location_corpus, content_transformer(replaceTags))
location_corpus <- tm_map(location_corpus, content_transformer(replaceURL))
location_corpus <- tm_map(location_corpus, content_transformer(replaceAmp))
location_corpus <- tm_map(location_corpus, content_transformer(replaceHashtags))
location_corpus <- tm_map(location_corpus, content_transformer(replaceUnicode))
location_corpus <- tm_map(location_corpus, content_transformer(replaceNewLine))
location_corpus <- tm_map(location_corpus, content_transformer(replaceContraction))
location_corpus <- tm_map(location_corpus, content_transformer(replaceMisc))
location_corpus <- tm_map(location_corpus, content_transformer(replaceEmoticons))
location_corpus <- tm_map(location_corpus, content_transformer(replacePunctuationForLocation))
location_corpus <- tm_map(location_corpus, removeNumbers)
location_corpus <- tm_map(location_corpus, content_transformer(removeSingleWords))
location_corpus <- tm_map(location_corpus, removeWords, stopwords("en"))
location_corpus <- tm_map(location_corpus, removeWhitespaces)

rlog::log_info("cleant location data.")
## inspect(location_corpus[1:100])
## ----------------------------------------------------------
df_clean_locations <- data.frame(text = sapply(location_corpus, as.character), stringsAsFactors = FALSE)
colnames(df_clean_locations) <- c("clean_locations")

df_clean_locations <- df_clean_locations %>% mutate_all(na_if, "")
...
updated_csv <- cbind(data, df_clean_tweets, df_clean_locations)
...
write.csv(updated_csv, "PreProcessed_Tweets.csv", append = FALSE)
